...
foreach($viewactivity as $cha){
    $estatus=$cha->status;
 }
mod_induction_view_activity($viewactivity,$estatus);
...
echo ('<div style="padding: 1px;"></div>');
mod_induction_view_newfunction($viewnewfunction, $estatus);
echo ('<div style="padding: 15px;"></div>');
/* Tabla procesos */
...
mod_induction_view_newprocesses($viewnewprocesses, $estatus);
...
mod_induction_view_newfile($viewnewfile, $estatus);

if (user_has_role_assignment($USER->id, 1) || is_siteadmin()){

}else if(user_has_role_assignment($USER->id, 4)){ 
echo ('<div style="padding: 15px;"></div>');
...
echo ('<div style="padding: 15px;"></div>');
    ...
    if(!empty($viewnewarea) && !empty($viewnewfunction) && !empty($viewnewprocesses)) {
        if($estatus==1){}else{
    echo ('<div style="padding: 15px;"></div>');
    ...
    echo   '<div class="modal fade" id="validar'.$id.'" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        <a href="validar_formato.php?id='.$id.'" class="btn btn-danger"><span class="glyphicon glyphicon-trash"></span>Validar</a>
                    </div>
