...
require_once('processes_form.php');
...
$processesform = new newprocesses_form();
$toform['idinstance'] = $idinstance;
$toform['courseid'] = $courseid;
$toform['idind'] = $id;
$processesform->set_data($toform);

$courseurl = new moodle_url('/mod/induction/viewact.php', array('courseid' => $courseid, 'idinstance' => $idinstance, 'id' => $id));
//Form processing and displaying is done here
if ($processesform->is_cancelled()) {

  redirect($courseurl);


    //Handle form cancel operation, if cancel button is present on form
} else if ($fromform = $processesform->get_data()) {
  //In this case you process validated data. $mform->get_data() returns data posted in form.


 //$contextmodule = context_module::instance($idinstance);

 
    $lastinsertid=$DB->insert_record('induction_processes', $fromform);
