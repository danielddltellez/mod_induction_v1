...
<?PHP
function xmldb_induction_upgrade( $oldversion = 0 )  { 

        // Agregar nuevos campos a la tabla de certificados. 
        $table  =  new xmldb_table ( 'induction' ) ; 
        ...
        $table  =  new xmldb_table ( 'induction_issues' ) ; 
        $field  =  new xmldb_field ( 'código' ) ; 
        $campo->set_attributes ( XMLDB_TYPE_CHAR ,  '50' ,  null ,  null ,  null ,  null ,  'inductionid' ) ; 
        if  ( !$dbman->field_exists ($table ,  $field ) )  { 
            $dbman->add_field ( $table ,  $field ) ; 
        }
 
        // Se ha alcanzado el punto de salvaguarda del certificado. 
        upgrade_mod_savepoint ( true ,  2019040300  ,  'induction' ) ; 
