...
class newinduction_form extends moodleform {
    ...
    function definition() {
        ...
        $sql="SELECT us.id as 'idusuario', CONCAT(us.firstname,' ', us.lastname) as 'nombre', mc.id as 'idcohorte' , mc.name as 'nombrecohorte', mcm.id as 'idmiembro' 
        FROM {user} us
        ...
        $mform->addRule('noempleado', null, 'numeric', null, 'client');
        $mform->addRule('noempleado', null, 'nopunctuation', null, 'client');  
        
        $select = $mform->addElement('select', 'idjefedirecto', 'Jeje directo', $options);
        $select->setSelected('1');
        $mform->addRule('idjefedirecto', null, 'required', null, 'client');
